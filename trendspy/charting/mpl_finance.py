...
def __infer_series_frequency(series):
    """
...
def ohlc_plot(ohlc: pd.DataFrame, width=0, colorup='g', colordown='r'):
    """
    Plot OHLC data frame

    :param ohlc: index, 'open', 'high', 'low', 'close' columns
    :param width: used bar width
    :param colorup: color of growing bar [green by default]
    :param colordown: color of declining bar [red by default]
    :return: axis
    """
    ...
    is_eod = __infer_series_frequency(ohlc_f) >= datetime.timedelta(1)
    ...
    if is_eod:
        ...
        ax.set_xticklabels([datetime.date.strftime(num2date(x), '%H:%M:%S') for x in ax.get_xticks()])
    f.autofmt_xdate()

    return ax


def to_mdate_index(x):
